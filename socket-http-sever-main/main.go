...
const (
	PORT           = 80
	ROOT           = "./static/"
	FLAG_PORT_HELP = "Port to listen to http requests."
	NOT_FOUND_MSG  = "404 Not Found\r\n"
	BAD_REQ_MSG    = "400 Bad Request\r\n"
)

var (
	HEADER_REGEX_1      = regexp.MustCompile(`^(GET) (/[\w\./]*) HTTP/\d\.\d$`)
	flagPort       *int = flag.Int("port", PORT, FLAG_PORT_HELP)
)
...
func CreateRequest(scanner *bufio.Scanner) (*HttpRequest, error) {
	...
	// Read first line in http request
	firstLine := scanner.Text()
	if firstLine == "" {
		return nil, errors.New("first line is empty")
	...
	log.Printf("Resource's path: %s", req.Path)

	// Read headers in http request
	...
	for scanner.Scan() {
		...
		log.Printf("%s %s", header[0], header[1])
...
func writeResponse(res *HttpResponse, writer *bufio.Writer) error {
	...
	headers += "Server: SocketHTTPServer/1.0\r\n"
